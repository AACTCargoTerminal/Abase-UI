...
export default function FsuRct({
  param,
  onClose,
  pgmId,
  headerAction,
    ...
    <div className="grid grid-cols-2 p-[2%] gap-y-3 gap-x-5">
      ...
      <div className="mainInput">
        <CommonDatePicker
          id={`Date`}
          title="Accepted Date"
          value={dt?.["RCT_ACTUAL_DATE"] || ""}
          onClick={(v) => setDt((prev) => ({ ...prev, RCT_ACTUAL_DATE: v }))}
          colSize="40%"
        />
      </div>
      <div className=" mainInput">
        {" "}
        <TimeInput
          value={dt?.["RCT_ACTUAL_TIME"]}
          id="Time"
          onChange={(v) => {
            setDt((prev) => ({ ...prev, RCT_ACTUAL_TIME: v }));
          }}
          label="Time"
          labelW="40%"
        />
      ...
      <div className="mainInput">
        <CommonInput
          id={`Transfer Carrier`}
          value={dt?.["CARRIER_CODE"] || ""}
          onChange={(v) => {
            setDt((prev) => ({ ...prev, CARRIER_CODE: v }));
          }}
      ...
      <div className="mainInput">
        <CommonInput
          id={`Airport`}
          value={dt?.["AIRPORT_CODE"] || ""}
          onChange={(v) => {
            setDt((prev) => ({ ...prev, AIRPORT_CODE: v }));
          }}
          label={`Airport`}
          labelW="40%"
        />
