...
export default function FsuRcs({
  param,
  onClose,
  pgmId,
  headerAction,
    ...
    <div className="grid grid-cols-2 p-[2%] gap-y-3 gap-x-5">
      ...
      <div className="mainInput">
        <CommonDatePicker
          id={`Date`}
          title="Accepted Date"
          value={dt?.["RCS_ACTUAL_DATE"] || ""}
          onClick={(v) => setDt((prev) => ({ ...prev, RCS_ACTUAL_DATE: v }))}
          colSize="40%"
        />
      </div>
      <div className=" mainInput">
        {" "}
        <TimeInput
          value={dt?.["RCS_ACTUAL_TIME"]}
          id="Time"
          onChange={(v) => {
            setDt((prev) => ({ ...prev, RCS_ACTUAL_TIME: v }));
          }}
          label="Time"
          labelW="40%"
        />
