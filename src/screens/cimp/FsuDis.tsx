...
import { getApi, getClass, sendLoading } from "../../Util/Util";
import { Divider } from "../../comp/Common";
import { CommonDatePicker, CommonDropDown } from "../../comp/DropDown";
import { commonHeader, fsuTimeHeader } from "../../Util/Header";
...
export default function FsuDis({
  param,
  onClose,
  pgmId,
  headerAction,
  ...
  const [discrepancy, setDiscrepancy] = useState<TableRow[]>([]);
  useEffect(() => {
    common();
  }, []);
  useEffect(() => {
  ...
  async function common() {
    const data = await getClass("1.8", pgmId);
    setDiscrepancy(data);
  }
    ...
    <div className="grid grid-cols-2 p-[2%] gap-y-3 gap-x-5">
      ...
      <div className="mainInput">
        <CommonDatePicker
          ...
          title="Date"
          value={dt?.["DIS_DATE"] || ""}
          onClick={(v) => setDt((prev) => ({ ...prev, DIS_DATE: v }))}
          colSize="40%"
        />
      </div>
      <div className=" mainInput">
        {" "}
        <TimeInput
          value={dt?.["DIS_ACTUAL_TIME"]}
          id="Time"
          onChange={(v) => {
            setDt((prev) => ({ ...prev, DIS_ACTUAL_TIME: v }));
          }}
          label="Time"
          labelW="40%"
        />
      ...
      <div className="mainInput">
        <CommonInput
          id={`RcfPcs`}
          value={dt?.["QTY_NO_OF_PIECES_DIS"] || ""}
          onChange={(v) => {
            setDt((prev) => ({ ...prev, QTY_NO_OF_PIECES_DIS: v }));
          }}
          label={`Pcs`}
          labelW="40%"
        />
      ...
      <div className="mainInput">
        <CommonInput
          id={`Airport`}
          value={dt?.["DIS_AIRPORT_CODE"] || ""}
          onChange={(v) => {
            setDt((prev) => ({ ...prev, DIS_AIRPORT_CODE: v }));
          }}
          label={`Airport`}
          labelW="40%"
        />
      ...
      <div className="mainInput">
        <CommonDropDown
          ...
          header={commonHeader}
          onClick={(r) => {
            setDt((prev) => ({ ...prev, DIS_CODE: r["CODE_CODE"] }));
          }}
          inputKey={{
            key: "CODE_CODE",
            showKey: "0",
      ...
      <div className="col-span-2 mainInput">
        <CommonInput
          id={`oth`}
          value={dt?.["OSI_DETAILS_1"] || ""}
          onChange={(v) => {
            setDt((prev) => ({ ...prev, OSI_DETAILS_1: v }));
          }}
          label={`Other Service Info`}
          labelW="20%"
        />
      </div>
    </div>
