...
export default function FsuRcf({
  param,
  onClose,
  pgmId,
  headerAction,
  ...
  return (
    <div className="grid grid-cols-2 p-[2%] gap-y-3 gap-x-5">
      ...
      <div className="mainInput">
        <CommonInput
          id={`Arr Airport`}
          value={dt?.["ARRIVAL_AIRPORT_CODE"] || ""}
          onChange={(v) => {
            setDt((prev) => ({ ...prev, ARRIVAL_AIRPORT_CODE: v }));
          }}
          label={`Arr.Airport`}
          labelW="40%"
        />
      ...
      <div className="mainInput">
        <CommonDatePicker
          id={`Date`}
          title="Date"
          value={dt?.["ARRIVAL_DATE"] || ""}
          onClick={(v) => setDt((prev) => ({ ...prev, ARRIVAL_DATE: v }))}
          colSize="40%"
        />
      </div>
      <div className=" mainInput">
        {" "}
        <TimeInput
          value={dt?.["RCF_ACTUAL_TIME"]}
          id="Time"
          onChange={(v) => {
            setDt((prev) => ({ ...prev, RCF_ACTUAL_TIME: v }));
          }}
          label="Time"
          labelW="40%"
        />
      ...
      <div className="mainInput">
        <CommonInput
          id={`RcfPcs`}
          value={dt?.["QTY_NO_OF_PIECES_RCF"] || ""}
          onChange={(v) => {
            setDt((prev) => ({ ...prev, QTY_NO_OF_PIECES_RCF: v }));
          }}
          label={`Pcs`}
          labelW="40%"
        />
